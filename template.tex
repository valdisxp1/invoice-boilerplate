...
\begin{spreadtab}{{tabular}[t t t]{lp{8.2cm}r}}
  ...
  $endif$
  $if(payment-currency)$
  @ & @ \multicolumn{1}{r}{Total ($currency$):}   & :={$if(VAT)$[0,-1]+[0,-2]$else$sum(c1:[0,-1])$endif$} \\ \hhline{~~-}
  @ & @ \multicolumn{1}{r}{$exchange-rate$  $payment-currency$ = 1 $currency$}   &  \\
  @ & @ \multicolumn{1}{r}{\textbf{Amount Due: }}   & \textbf{:={[0,-2]/$exchange-rate$} $payment-currency$} \\ \hhline{~~-}
  $else$
