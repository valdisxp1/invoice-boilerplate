...
\begin{spreadtab}{{tabular}[t t t]{lp{8.2cm}r}}
  ...
  $endif$
  @ & @ \multicolumn{1}{r}{\textbf{Total in $currency$:}}   & \textbf{:={$if(VAT)$[0,-1]+[0,-2]$else$sum(c1:[0,-1])$endif$}} \\ \hhline{~~-}
  $if(payment-currency)$
  @ & @ \multicolumn{1}{r}{1 $payment-currency$ = $exchange-rate$ $currency$:}   &  \\
  @ & @ \multicolumn{1}{r}{\textbf{Total in $payment-currency$:}}   & \textbf{:={[0,-2]*$exchange-rate$}} \\ \hhline{~~-}
  $endif$
\end{spreadtab}
